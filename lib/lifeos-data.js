...
const LEGACY_WORK_PROJECT_NAMES = {
...
export function migrateState(rawState) {
  ...
  const normalizeNoteItem = (item, index = 0) => {
    ...
    return {
      ...
      title,
      content,
      type: typeof item.type === "string" && item.type ? item.type : "Draft",
      pinned: Boolean(item.pinned),
      archived: Boolean(item.archived),
      createdAt: item.createdAt || new Date(0).toISOString(),
      updatedAt: item.updatedAt || item.createdAt || new Date(0).toISOString(),
      colorIndex: Number.isFinite(Number(item.colorIndex)) ? Number(item.colorIndex) : index % 10
    };
  };
  ...
  const merged = {
    ...
    noteItems: rawNoteItems.map(normalizeNoteItem).filter(Boolean),
...
export function choosePreferredSyncState(localState, remoteState) {
  ...
  if (localHasUnsyncedChanges && !remoteIsNewerThanLocalSync) return local;
  if (!localHasUnsyncedChanges && remoteIsNewerThanLocalSync) return remote;
