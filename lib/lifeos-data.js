  ...
  return {
    ...
    workProjects: DEFAULT_STATE.workProjects.map((project) => {
      ...
      return {
        ...project,
        ...sourceProject,
        todos: (sourceProject.todos ?? []).filter((todo) => !LEGACY_WORK_TODO_IDS.has(todo.id))
      };
    })
  };
}
...
export function getStateContentScore(state) {
  ...
  const totalTodos = (state?.workProjects ?? []).reduce((sum, project) => sum + (project.todos?.length ?? 0), 0);
  const totalXP = Number(state?.profile?.totalXP ?? 0);
  return logsCount * 10 + totalTodos * 3 + totalXP;
}
