...
export function migrateState(rawState) {
  ...
  const incomingProjects = rawState?.workProjects ?? DEFAULT_STATE.workProjects;
  ...
  return {
    ...
    workProjects: DEFAULT_STATE.workProjects.map((project) => {
      ...
      const legacyTodos = (sourceProject.todos ?? []).filter((todo) => !LEGACY_WORK_TODO_IDS.has(todo.id)).map(normalizeItem);
      const todayActions = (sourceProject.todayActions ?? []).map(normalizeItem);
      const backlog = (sourceProject.backlog ?? []).map(normalizeItem);
      const nextDayCandidates = (sourceProject.nextDayCandidates ?? []).map(normalizeItem).slice(0, 5);
